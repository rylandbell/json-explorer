...
import reducers from '../src/js/reducers.jsx';

test('action type: UPDATE_DATA', function(t) {
  var stateBefore = { data: {}, currentPath: [], showError: false, textContent: '', dataIsPreloaded: false, showError: false };
  var testAction = { type: 'UPDATE_DATA', data: { testProp: 'testValue' } };
  var stateAfter = { data: { testProp: 'testValue' }, currentPath: [], showError: false, textContent: '', dataIsPreloaded: false, showError: false };

  deepFreeze(stateBefore);
  deepFreeze(testAction);

  t.deepEqual(reducers.explorerApp(stateBefore, testAction), stateAfter, 'should add data to initial state');
  t.end();
});
...
// var Reducers = require('../src/js/reducers.jsx');
